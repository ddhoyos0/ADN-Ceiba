...
public class PersonaControladorTest {
   ...
   /*
    ...
    public void actualizarPersona() throws Exception{
        ComandoPersona comando = datosBuilder.buildComandoPersona();
        MvcResult mvcResult = mockMvc.perform(put("/persona/actualizar")
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(comando)))
                .andReturn();
...
*/
    ...
    public void ConsultaTipoDocumento() throws Exception {
        MvcResult mvcResult = mockMvc.perform(get("/persona/documento")
                .contentType(MediaType.APPLICATION_JSON)
                .accept(MediaType.APPLICATION_JSON))
                .andReturn();
        int status = mvcResult.getResponse().getStatus();
	...
	public void eliminarPersona() throws Exception {
		MvcResult mvcResult = mockMvc
				.perform(delete("/persona/eliminar/1").contentType(MediaType.APPLICATION_JSON).accept(MediaType.APPLICATION_JSON))
				.andReturn();
		int status = mvcResult.getResponse().getStatus();
		Assertions.assertEquals(200, status);
